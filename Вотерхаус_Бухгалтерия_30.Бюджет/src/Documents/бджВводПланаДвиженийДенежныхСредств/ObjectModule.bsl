...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Движения.бджПланДвиженийДенежныхСредств.Записывать = Истина;
	Для Каждого ПланСтрока Из ДанныеДляПроведения.План Цикл
		Движение = Движения.бджПланДвиженийДенежныхСредств.Добавить();
...
Функция ДанныеДляПроведенияДокумента()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	План.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
	|	СУММА(План.Сумма) КАК Сумма,
	|	План.Ссылка КАК Регистратор,
	|	План.Ссылка.Месяц КАК Период,
	|	План.Ссылка.Организация
	|ИЗ
	|	Документ.бджВводПланаДвиженийДенежныхСредств.План КАК План
	|ГДЕ
	|	План.Ссылка = &Ссылка
	|СГРУППИРОВАТЬ ПО
	|	План.СтатьяДвиженияДенежныхСредств,
	|	План.Ссылка,
	|	План.Ссылка.Месяц,
	|	План.Ссылка.Организация";
