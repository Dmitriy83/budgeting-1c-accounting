...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета) Экспорт 
	...
	Если ПараметрыОтчета.КлючТекущегоВарианта = "АнализДвиженийДенежныхСредств" Тогда
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	// Бюджет
	// Заполним текст запроса Бюджет программно, иначе отчет в Конфигураторе нельзя будет нормально редактровать ни как внешний, ни в расширении,
	// т.к. часть использующихся объектов находится только в расширениию, а часть только в основной конфигурации.
	бджЗаполнитьТекстЗапросаБюджет(Схема);
	...
	Если Таблица <> Неопределено Тогда
		...
		Если СтрНайти(ПараметрыОтчета.КлючТекущегоВарианта, "ПоПериодам") Тогда
...
Функция ДобавитьОбязательныеГруппировки(Группировка, ПараметрыОтчета, ПолеПериода)
	...
	Если ПараметрыОтчета.Периодичность = 0 Тогда
		...
		ПараметрыОтбора.Вставить("ЗначениеОтбора", ПараметрыОтчета.НачалоПериода);
...
Процедура бджЗаполнитьТекстЗапросаБюджет(СКД)
	
	НаборДанныхБюджет = СКД.НаборыДанных.НаборДанных1.Элементы.Найти("Бюджет");
	Если НаборДанныхБюджет <> Неопределено Тогда
		НаборДанныхБюджет.Запрос = 
		"ВЫБРАТЬ
		|	бджПланДвиженийДенежныхСредствОбороты.Организация,
		|	бджПланДвиженийДенежныхСредствОбороты.СтатьяДвиженияДенежныхСредств КАК СтатьяДвиженияДенежныхСредств,
		|	бджПланДвиженийДенежныхСредствОбороты.СтатьяДвиженияДенежныхСредств.ВидДвиженияДенежныхСредств КАК
		|		ВидДенежныхСредств,
		|	Выбор
		|		Когда бджПланДвиженийДенежныхСредствОбороты.СуммаОборот >= 0
		|			Тогда ""Поступление""
		|		Иначе ""Списание""
		...
		|			Тогда 1
		|		Иначе -1
		|	Конец * бджПланДвиженийДенежныхСредствОбороты.СуммаОборот КАК СуммаБюджет,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодЧас,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодСекунда,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодПолугодие,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодНеделя,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодМинута,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодМесяц,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодКвартал,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодДень,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодДекада,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодГод,
		|	бджПланДвиженийДенежныхСредствОбороты.ПериодСекунда КАК Период
		|ИЗ
		|	РегистрНакопления.бджПланДвиженийДенежныхСредств.Обороты(&НачалоПериода, &КонецПериода, Авто,) КАК
		|		бджПланДвиженийДенежныхСредствОбороты";
	КонецЕсли;
